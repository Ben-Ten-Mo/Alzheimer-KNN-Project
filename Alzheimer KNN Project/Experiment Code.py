...
'''
features = ['Age','EDUC','MMSE','eTIV','ASF']
X = patients.loc[:, features]
print("X: %s"%X)
y = patients.loc[:, ['Group']]
print("y: %s"%y)
'''
#print(patient_aspects)
...
#Either This
kmeans.fit(patient_aspects.drop(['Group'],axis=1))
X = patient_aspects.drop(['Group'],axis=1)
#print("Working X: \n%s"%X)
y = kmeans.predict(patient_aspects.drop(['Group'],axis=1))
#print("Working y: \n%s"%y)


'''
#Or This, need closer examination
X = patient_aspects.drop(['Group'],axis=1)
y = y_kmeans
'''
...
#patients_2 = patients_2.drop(['label'],axis=1)
...
definitions = ['Nondemented','Demented']
